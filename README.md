...
Changed words are highlighted within each modified paragraph or table row; a paragraph that was largely rewritten is tinted instead. When a word changes only partly (a plural, a tense, punctuation), just the changed letters are marked. Formatting-only changes (same text, e.g. bold added or a style changed) and case-only changes are marked separately, as cosmetic. Use `n` / `p` to jump between changes, or "changes only" to fold unchanged text.
