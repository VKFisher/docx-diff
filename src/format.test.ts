...
describe('formatDiff', () => {
  it('sees no change when only run boundaries differ', () => {
    ...
    expect(formatDiff(a, 'docx-old', b, 'docx-new')).toEqual({ a: [], b: [] });
  });
  ...
  it('marks the words whose formatting changed', () => {
    ...
    const diff = formatDiff(a, 'docx-old', b, 'docx-new')!;
  ...
  it('marks everything when the paragraph style changes', () => {
    ...
    expect(marked(b, formatDiff(a, 'docx-old', b, 'docx-new')!.b)).toEqual(['Beds']);
  });
  ...
  it('returns null when the texts differ', () => {
    expect(formatDiff(p('abc'), 'docx-old', p('abcd', 'docx-new'), 'docx-new')).toBeNull();
  });
});
